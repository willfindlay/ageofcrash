...
impl ConfigWatcher {
    pub fn new<P: AsRef<Path>>(config_path: P) -> Result<(Self, Receiver<ConfigEvent>), Box<dyn std::error::Error>> {
            ...
            while !should_stop.load(Ordering::Relaxed) {
                match std::fs::metadata(&path) {
                    ...
                    Err(e) => {
                        ...
                        if tx.send(ConfigEvent::Error(format!("File access error: {}", e))).is_err() {
                            break;
                        }
                    }
                }
